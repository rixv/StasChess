    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (t = -6) and (abs(a - i) = 1) and (abs(b-j) = 1)  and ((n <> 0) or (m <> 0)) then
          begin
            Result := True;
            exit;
          end;
...
//  for i := 1 to Result.Count do Result.Move[i].Sort := 0;
 ...
 //  Result := CResult;
  ...
  begin
   ...
   begin
     Smoves := SMoves + ConvertMoveToStr(Result.Move[i]) + '-' + IntToStr(Result.Move[i].Sort) + #13;
   end;
...
///  if  Moves.Move[i].Sort < 30 then
    Moves.Move[i].Sort := Moves.Move[i].Sort + ((HashTable[GetHash(DoMove(Board, Moves.Move[i]))].Val));
    Result := Moves;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      2: Result := Result + KnightMoveCount(Board, i, j)*2;
